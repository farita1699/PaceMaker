...
class GraphWindow(PlotWidget):
    ...
    def __init__(self, plot: PlotWidget):
        ...
        self.plot = plot.plot(pen=(0, 229, 255))
        self._plot()


    def _plot(self):
        time_period = 10
        plot_wid = self.plot.getViewWidget()
        plot_wid.disableAutoRange()
        if self.maxvals <= 400:
            size = len(self._atrial_data)
            x = np.arange(0, size) / time_period
            self.plot.setData(x, self._atrial_data)
            size = size/time_period
            if size % 5 == 0:
                plot_wid.setXRange(size-4,size+6)
                plot_wid.enableAutoRange(x=False, y=True)
        else:
            x = np.arange(self.maxvals - 400, self.maxvals) / time_period
            self.plot.setData(x, self._atrial_data)
            position = self.maxvals/time_period
            if position % 5 == 0:
                plot_wid.setXRange(position-4,position+6)
                plot_wid.enableAutoRange(x=False, y=True)
